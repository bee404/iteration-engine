...
/**
 * Model-agnostic critique/directions logic shared by every real LLMProvider implementation
 * (ClaudeLLMProvider primary, OpenAILLMProvider fallback — see docs/decisions.md). Tool
 * schemas, prompts, and response validation live here once so the two backends can never
 * silently drift apart on what counts as a valid Critique/Direction[]; only the wire format
 * (Anthropic `tool_choice` + content blocks vs. OpenAI `tool_choice` + `tool_calls`) differs,
 * and that stays in each provider's own file.
 */

/** One retry on a failed or unparseable response, per the product decision to surface a
 * typed error rather than crash instead of retrying indefinitely. Applies identically to
 * the primary and fallback backends so a caller can't tell which one produced a result from
 * how many attempts it took. */
export const MAX_ATTEMPTS = 2;

export const SUBMIT_CRITIQUE_TOOL = "submit_critique";
export const SUBMIT_DIRECTIONS_TOOL = "submit_directions";
...
export const REQUIRED_DIRECTIONS = 3;
...
/**
 * Forcing this tool via each backend's forced tool-choice equivalent makes the model return
 * the critique as validated JSON instead of free-form prose, which is most of what "parsing
 * the response" would otherwise have to guard against. `kind` is deliberately not part of the
 * schema: it's derived from which array an item is in (see `toItems`) so a mislabeled field
 * from the model can never desync signal/preference from their stated meaning.
 */
...
export const CRITIQUE_INPUT_SCHEMA = {
  type: "object",
  properties: {
    summary: {
      type: "string",
...
/**
 * Forcing this tool makes the model return the directions as validated JSON rather than prose.
...
export function resolveScreenshot(screenshotRef: string) {
  try {
    ...
    if (error instanceof ScreenshotValidationError) {
      logSecurityEvent("screenshot_rejected", { provider: "critique" });
      throw new CritiqueGenerationError("invalid_screenshot", error.message);
    }
    throw error;
  }
}

export function buildPrompt(request: CritiqueRequest): string {
...
function toItems(kind: SignalPreferenceItem["kind"], values: unknown): SignalPreferenceItem[] {
  if (!Array.isArray(values)) {
    throw new CritiqueGenerationError("unparseable_response", `Expected "${kind}" to be an array in the model's response.`);
  }
...
export function parseToolInput(input: unknown, model: string): Critique {
  if (!input || typeof input !== "object") {
    throw new CritiqueGenerationError("unparseable_response", "The model's tool call input was not an object.");
  }
  const record = input as Record<string, unknown>;
  if (typeof record.summary !== "string" || !record.summary.trim()) {
    throw new CritiqueGenerationError("unparseable_response", 'The model\'s response is missing a non-empty "summary".');
  }
  if (!Array.isArray(record.flaggedAmbiguities) || !record.flaggedAmbiguities.every((item) => typeof item === "string")) {
    throw new CritiqueGenerationError(
      ...
      '"flaggedAmbiguities" in the model\'s response must be an array of strings.'
    );
  }
...
export function buildDirectionsPrompt(request: DirectionsRequest): string {
...
export function toDirections(input: unknown, patternReferences: PatternReference[]): Direction[] {
  if (!input || typeof input !== "object") {
    throw new DirectionsGenerationError("unparseable_response", "The model's tool call input was not an object.");
  }
  const { directions } = input as { directions?: unknown };
  if (!Array.isArray(directions)) {
    throw new DirectionsGenerationError("unparseable_response", 'The model\'s response is missing a "directions" array.');
  }
  if (directions.length !== REQUIRED_DIRECTIONS) {
    throw new DirectionsGenerationError(
      "unparseable_response",
      `Expected exactly ${REQUIRED_DIRECTIONS} directions but the model returned ${directions.length}.`
    );
  }
...
function assertDistinct(directions: Direction[]): void {
  ...
  for (const direction of directions) {
    ...
    if (seenRationales.has(rationaleKey) || seenChangeSets.has(changesKey)) {
      throw new DirectionsGenerationError(
        "unparseable_response",
        "The model returned directions that share an identical rationale or suggested-changes list; they are not substantively distinct."
      );
    }
