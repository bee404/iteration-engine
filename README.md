...
The current V0 flow is screenshot upload and intake, Claude critique and directions, one selected direction, streamed code generation, a fixed-box `Source` / `Iteration` review, and a client-generated ZIP. Code generation preserves the screenshot's existing visual system by default and receives the complete round context plus locked viewport; restyling with a design system or performing a broader redesign requires an explicit generation mode. The ZIP contains a runnable Vite/React prototype plus `coqui-context.json` with the raw inputs, synthesized critique, full selected direction, viewport, generation notes, and the provider/model that actually completed generation. The processed screenshot remains transient browser-session state and is excluded from the download by default.
